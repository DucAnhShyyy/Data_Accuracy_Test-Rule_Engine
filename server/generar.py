...
def generate_sql():
    try:
        ...
        try:
            ...
            return jsonify({"message": "SQL scripts generated and saved to sql_script.json"}), 200
        except Exception as e:
            return jsonify({"message": f"Error saving file: {str(e)}"}), 500
    except Exception as e:
        return jsonify({"message": f"Error generating SQL: {str(e)}"}), 500
...
def generate_complete_sql():
    try:
        rules_data = load_rules_data()
        if not rules_data:
            return jsonify({"message": "No rules data available to generate SQL"}), 404
        complete_sql = []
        ...
        try:
            with open(SQL_SCRIPT_FILE, 'w', encoding='utf-8') as f:
                f.write(sql_script)
            return jsonify({"message": "Complete SQL script generated and saved to sql_script.json"}), 200
        except Exception as e:
            return jsonify({"message": f"Error saving file: {str(e)}"}), 500
    except Exception as e:
        return jsonify({"message": f"Error generating SQL: {str(e)}"}), 500
